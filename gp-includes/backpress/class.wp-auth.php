...
class WP_Auth
{
	...
	function validate_auth_cookie( $cookie = null, $scheme = 'auth' )
	{
		...
		if( $element_count == 3 ) {
			...
			$auth_args['hmac'] = $cookie_elements[2];
		}
		else {
			$auth_args = apply_filters( 'gp_auth_cookie_parse', false, $cookie_elements );
	...
	function clear_auth_cookie()
	{
		...
		$cookies = apply_filters( 'gp_auth_clear_cookies', $this->cookies );
		foreach ( $cookies as $_scheme => $_scheme_cookies ) {
